...
test('release root and version use existing correct parser and prediction scripts',()=>{
 ...
 for(const dir of [root,path.join(root,'versions/v2.17.0')]){
  ...
  assert.match(scripts[0],/parser(?:-v2\.17\.0)?\.js/);assert.match(scripts[2],/mox-rewards(?:-v2\.17\.0)?\.js/);assert.match(scripts[1],/rewards(?:-v2\.17\.0)?\.js/);
 }
});
