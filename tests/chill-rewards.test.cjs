...
const R=require('../versions/v2.18.0/rewards.js'),P=require('../versions/v2.18.0/parser.js');
...
function app(){
 const html=fs.readFileSync(require.resolve('../versions/v2.18.0/index.html'),'utf8');
 const js=[...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(x=>x[1]).join('\n').replace(/boot\(\);\s*$/,'');
 const memory=new Map(),c={ChillRewards:R,StatementParser:P,GoRewards:require('../versions/v2.18.0/go-rewards'),MoxRewards:require('../versions/v2.18.0/mox-rewards.js'),console,Date,setTimeout:()=>0,clearTimeout:()=>{},localStorage:{getItem:k=>memory.get(k)||null,setItem:(k,v)=>memory.set(k,v)},document:{querySelector:()=>null,addEventListener:()=>{}},window:{addEventListener:()=>{}}};
...
test('release root and version use existing correct parser and prediction scripts',()=>{
 ...
 for(const dir of [root,path.join(root,'versions/v2.18.0')]){
  const html=fs.readFileSync(path.join(dir,'index.html'),'utf8');
  const scripts=[...html.matchAll(/<script src="(\.\/[^\"]+)"/g)].map(m=>m[1]);assert.equal(scripts.length,4);
  for(const file of scripts)assert.ok(fs.existsSync(path.join(dir,file)),file);
  assert.match(scripts[0],/parser(?:-v2\.18\.0)?\.js/);assert.match(scripts[2],/mox-rewards(?:-v2\.18\.0)?\.js/);assert.match(scripts[1],/rewards(?:-v2\.18\.0)?\.js/);
 }
});
