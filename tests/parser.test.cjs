...
const P=require('../versions/v2.18.0/parser.js');
const ChillRewards=require('../versions/v2.18.0/rewards.js');
...
test('statement month, not each transaction calendar month, controls the consumption-record column',()=>{
 ...
 assert.equal(r.meta.record_month,'2026-08');assert.deepEqual(r.rows.map(x=>x.record_month),['2026-08','2026-08']);
});
...
function app(){
 const h=fs.readFileSync(require.resolve('../versions/v2.18.0/index.html'),'utf8');
 const js=[...h.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(m=>m[1]).join('\n').replace(/boot\(\);\s*$/,'');
 const memory=new Map();const c={document:{querySelector:()=>null,addEventListener:()=>{}},window:{addEventListener:()=>{}},StatementParser:P,ChillRewards,GoRewards:require('../versions/v2.18.0/go-rewards'),MoxRewards:require('../versions/v2.18.0/mox-rewards.js'),Date,console,localStorage:{getItem:k=>memory.get(k)||null,setItem:(k,v)=>memory.set(k,v)},setTimeout:()=>0,clearTimeout:()=>{}};
...
test('bank-declared New Charges wins and separate reward and other panels keep accounting order',()=>{
 const c=app();vm.runInContext(`S.data.statementImports=[{fp:'s',name:'chill卡 2024年08 月.pdf',meta:{record_month:'2024-08',statement_month:'2024-08'},reviewed_rows:[{cardId:'card-boc-chill'}],checks:[{currency:'HKD',declared_debits:561386}]}];S.data.transactions=[{id:'t',fp:'s',date:'2024-07-19',record_month:'2024-08',cardId:'card-boc-chill',amount:5335.21,merchant:'SHOP',category:'other'}];S.data.rewardMonths=[{id:'cash',cardId:'card-boc-chill',month:'2024-08',amount:150},{id:'pts',cardId:'card-boc-chill',month:'2024-08',amount:0,reward_unit:'gift_points',earned:250,valuation_hkd:1},{id:'mile',cardId:'card-boc-chill',month:'2024-08',amount:0,reward_unit:'asia_miles',earned:100,valuation_hkd:6}];S.data.credits=[{id:'off',date:'2024-08-02',record_month:'2024-08',cardId:'card-boc-chill',amount:7,kind:'reward_offset',merchant:'OFFSET'},{id:'pay',date:'2024-08-08',record_month:'2024-08',cardId:'card-boc-chill',amount:100,kind:'payment',merchant:'PAY'},{id:'fee',fp:'s',date:'2024-08-07',record_month:'2024-08',cardId:'card-boc-chill',amount:.69,kind:'fee',merchant:'FEE'}];UI.recMonth='2024-08';UI.recCard='card-boc-chill';UI.recCat='all';`,c);
 const tx=vm.runInContext('recordsHTML()',c);assert.match(tx,/HK\$5,613\.86/);assert.match(tx,/chill卡 2024年08 月\.pdf/);vm.runInContext("UI.recTab='rm'",c);const rm=vm.runInContext('recordsHTML()',c);assert.doesNotMatch(rm,/<h2 class="p-title">月結回贈/);assert.match(rm,/HK\$163\.00/);const section=rm.slice(rm.indexOf('<section class="panel" data-section="rewards">'));const list=section.slice(section.indexOf('<div class="row"'));for(const [a,b] of [['現金回贈','積分回贈'],['積分回贈','里程回贈'],['里程回贈','獎賞兌換'],['還款','費用']])assert.ok(list.indexOf(a)<list.indexOf(b));
});

test('reimport refreshes corrected card/date metadata but preserves existing edits and actual reward',async()=>{
 const c=app();c.text='BOC Dual Currency Platinum Card\n結單日期\n18-NOV-2024\nStatement Date\n到期付款日\n13-DEC-2024\nHKD Account No.: 1111-2222-3333-4444\n16-NOV 15-NOV SHOP 10.00';
 const again=async()=>{vm.runInContext('IMPPDF.rows=buildRows(text);IMPPDF.stmts=[{fp:IMPPDF.rows[0].fp,name:"sample",text}];IMPPDF.pdfs=[];',c);await vm.runInContext('ACTIONS["pdft-save"]()',c);};
 await again();vm.runInContext('S.data.transactions[0].record_month="2024-12";S.data.transactions[0].cardId="card-boc-go-dia";S.data.transactions[0].merchant="Edited SHOP";S.data.transactions[0].amount=12;S.data.transactions[0].actualReward=1',c);await again();
 assert.equal(vm.runInContext('S.data.transactions.length',c),1);assert.equal(vm.runInContext('S.data.transactions[0].record_month',c),'2024-11');assert.equal(vm.runInContext('S.data.transactions[0].cardId',c),'card-boc-go');assert.equal(vm.runInContext('S.data.transactions[0].merchant',c),'Edited SHOP');assert.equal(vm.runInContext('S.data.transactions[0].amount',c),12);assert.equal(vm.runInContext('S.data.transactions[0].actualReward',c),1);
});
