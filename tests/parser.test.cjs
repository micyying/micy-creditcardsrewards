...
const P=require('../versions/v2.17.0/parser.js');
const ChillRewards=require('../versions/v2.17.0/rewards.js');
...
function app(){
 const h=fs.readFileSync(require.resolve('../versions/v2.17.0/index.html'),'utf8');
 const js=[...h.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(m=>m[1]).join('\n').replace(/boot\(\);\s*$/,'');
 const memory=new Map();const c={document:{querySelector:()=>null,addEventListener:()=>{}},window:{addEventListener:()=>{}},StatementParser:P,ChillRewards,GoRewards:require('../versions/v2.17.0/go-rewards'),MoxRewards:require('../versions/v2.17.0/mox-rewards.js'),Date,console,localStorage:{getItem:k=>memory.get(k)||null,setItem:(k,v)=>memory.set(k,v)},setTimeout:()=>0,clearTimeout:()=>{}};
