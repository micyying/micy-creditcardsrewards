...
const P=require('../versions/v2.14.0/parser.js');
...
test('dual dates, cross-year attribution, markers, fees and redemption remain separate',()=>{
 ...
 assert.equal(r.rows[0].transaction_date,'2025-12-31');assert.equal(r.rows[0].post_date,'2026-01-03');assert.equal(r.rows[0].merchant_marker,'##');assert.equal(r.rows[0].online,null);assert.equal(r.rows[1].cashback_period,'2025-12');assert.equal(r.rows[2].kind,'reward_offset');assert.equal(r.rows[3].kind,'fee');assert.equal(r.rows[4].kind,'odd_cent');assert.equal(r.rows[0].expected_cashback,null);
});
...
function app(){
 const h=fs.readFileSync(require.resolve('../versions/v2.14.0/index.html'),'utf8');
...
test('app import persists provenance, separates CNY, is idempotent and leaves old storage alone',async()=>{
 ...
 assert.equal(vm.runInContext('S.data.transactions.length',c),2);assert.equal(vm.runInContext('S.data.credits.length',c),1);assert.equal(vm.runInContext('S.data.statementImports.length',c),1);assert.equal(vm.runInContext('S.data.transactions[0].raw_description',c),'SHOP');assert.equal(vm.runInContext('S.data.transactions[0].expectedReward',c),null);assert.equal(c.localStorage.getItem('micy_shuana_data_v1'),null);
