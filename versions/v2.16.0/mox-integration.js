...
recalculatePredictions=function(data){
 let changed=preMoxRecalculate(data);
...
ACTIONS['mox-reconciliation']=()=>{
 ...
 openSheet(`<div class="s-head">Mox CashBack 對帳</div><p class="s-sub">消費按信用卡月結周期分欄；現金按銀行結算／入賬曆月核對。</p>${months.map(moxMonthHTML).join('')||'<p>尚未匯入Mox資料</p>'}<section class="panel"><b>迎新獎賞（獨立）</b>${welcomes.map(t=>`<div class="kv"><span>${esc(t.date)} · ${esc(t.merchant)}</span><b>${fmtHKD(t.amount)}</b></div>`).join('')||'<p class="mini">未取得迎新入賬紀錄</p>'}</section><details class="panel"><summary>日期／金額候選配對（${candidates.length}組）</summary><p class="mini">同日合計或延遲1–3日的金額候選，可能重疊；不等同銀行逐筆確認，不分配實際回贈。</p>${candidates.map(c=>`<p class="mini">${esc(c.date)}${c.observed_date?' → '+esc(c.observed_date):''} · ${c.transaction_ids.length}筆 · 預測${fmtHKD(c.scenario_cashback)}／實收${fmtHKD(c.observed_cashback)} · ${esc(c.status)}</p>`).join('')}</details>${ids.map(id=>`<button class="btn btn-ghost btn-block" data-action="mox-settings" data-id="${esc(id)}">${esc(cardById(id)?.name)} · 規則設定</button>`).join('')}`);
};
...
const preMoxRecords=recordsHTML;
recordsHTML=function(){const html=preMoxRecords();return (S.data.cards.some(c=>isMoxCard(c.id))&&(UI.recCard==='all'||isMoxCard(UI.recCard))?'<section class="panel"><button class="btn btn-ghost btn-block" data-action="mox-reconciliation">Mox 現金回贈 · 月度對帳／規則</button></section>':'')+html;};
