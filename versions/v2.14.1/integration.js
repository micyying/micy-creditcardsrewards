...
migrateData=function(){
 ...
 for(const st of statements.values()){
  const target=st.meta?.record_month||st.meta?.period_start?.slice(0,7)||(st.meta?.statement_month?monthAdd(st.meta.statement_month,-1):null);
 ...
 for(const r of [...(S.data.transactions||[]),...(S.data.credits||[])]){
  ...
  const st=statements.get(r.fp),target=st?.meta?.record_month||st?.meta?.period_start?.slice(0,7)||r.record_month||(r.statement_month?monthAdd(r.statement_month,-1):null);
  ...
  if(r.parser_version==='2.14.0'){r.parser_version='2.14.1-migrated';changed=true;}
 }
 return changed;
};
 ...
 try{
  ...
  for(const report of parsedReports()){
   ...
   for(const r of report.rewards){if(r.earned==null)continue;
    dd.rewardMonths.push({id:uid(),fp:report.fp,cardId:r.cardId,month:report.meta.record_month||report.meta.statement_month,amount:0,reward_unit:r.unit,earned:r.earned,valuation_hkd:r.valuation_hkd,parser_version:'2.14.1',note:'結單獎賞 · 與現金分開',sid:mine[0]?.sid});
   }
  }
...
settingsHTML=function(){return '<section class="panel"><b>v2.14.1 · 月結周期分欄</b><p class="mini">消費按月結周期起始月份分欄，交易日期仍按原單顯示；商戶名稱完整保留，包括 ##。v2.14.0 舊版仍保留。</p><a href="../v2.14.0/">開啟 v2.14.0</a></section>'+originalSettingsHTML();};
