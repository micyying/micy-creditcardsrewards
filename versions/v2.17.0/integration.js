...
function statementDeclaredSpend(month,cardId='all'){
 ...
 const selected=r=>(r.record_month||String(r.date||'').slice(0,7))===month&&(cardId==='all'||r.cardId===cardId)&&(!r.currency||r.currency==='HKD')&&!r.demo;
 const rows=[...(S.data.transactions||[]).filter(selected),...(S.data.credits||[]).filter(r=>selected(r)&&r.kind==='fee')];
 ...
 for(const st of statementImportsOfMonth(month,cardId)){
  ...
  for(const check of st.checks||[]){
   if(check.currency!=='HKD'||check.declared_debits==null)continue;
...
function rewardRecordsHTML(month,cardOpts){
 ...
 for(const c of credits){const isReward=['reward_offset','welcome_reward'].includes(c.kind);(isReward?rewardItems:otherItems).push({rank:isReward?(c.kind==='reward_offset'?3:4):(rank[c.kind]??5),date:c.date||'',id:c.id,html:creditSummaryRow(c)});}
...
recordsHTML=function(){let html=originalRecordsHTML();const cny=(S.data.credits||[]).filter(r=>r.kind==='foreign_tx'&&r.currency==='CNY'&&(r.record_month||r.date?.slice(0,7))===UI.recMonth&&(UI.recCard==='all'||r.cardId===UI.recCard));
 return `${cny.length?'<p class="mini">本月人民幣消費 '+cny.length+' 筆：CN¥'+r2(cny.reduce((s,r)=>s+r.amount,0))+'，與港幣總消費分開。</p>':''}`+html;};
