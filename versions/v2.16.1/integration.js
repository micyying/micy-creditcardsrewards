...
function migrateAccountRecords(data){
 ...
 for(const k of data.knowledge||[]){const name=k.pattern||k.merchant||k.keyword||'';if(/CIRCLE\s*[- ]?\s*K\b/i.test(name))set(k,'category','other');if(/RENT\s*SMART/i.test(name))set(k,'category','rentsmart');}
 return changed;
}
