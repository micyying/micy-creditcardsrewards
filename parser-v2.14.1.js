...
function meta(text){
 ...
 // 消費紀錄按月結周期起始月歸檔。Mox 有明示周期；其他信用卡以結單月份的上一月為周期月。
 if(m.period_start)m.record_month=m.period_start.slice(0,7);
 else if(m.statement_month){const [y,n]=m.statement_month.split('-').map(Number),d=new Date(Date.UTC(y,n-2,1));m.record_month=`${d.getUTCFullYear()}-${pad(d.getUTCMonth()+1)}`;}
 return m;
}
